...
bool MagixAutoLogin::enabled = false;
bool MagixAutoLogin::autoChar = false;
std::string MagixAutoLogin::user;
std::string MagixAutoLogin::pass;
...
// kind of dumb but let's allow it .. for now
	...
	extern "C" {
...
#endif
		{
			...
			// Parse launcher-provided auto-login args before the game starts.
			#if OGRE_PLATFORM == OGRE_PLATFORM_WIN32
			MagixAutoLogin::parse(strCmdLine);
			#else
			{
				std::string cmd;
				for (int i = 1; i < argc; ++i) { if (i > 1) cmd += ' '; cmd += argv[i]; }
				MagixAutoLogin::parse(cmd.c_str());
			}
			#endif
