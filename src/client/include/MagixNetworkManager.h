...
public:
	...
	void update(const FrameEvent evt)
	{
		...
		case ID_SETADMIN: processSetAdmin(packet); break;
		case ID_SETRANK: processRankPush(packet); break;
		...
		case ID_ANNOUNCE: processAnnounce(packet); break;
			default:
	...
	{
		// Version authority is the updater: version.txt is synced from the
		// update manifest (see manifest_build). The compiled GAME_VERSION
		// is only a fallback for dev runs without the update flow.
		std::ifstream tFile("version.txt");
		if(tFile.good())
		{
			std::string tLine;
			std::getline(tFile,tLine);
			while(!tLine.empty() && (tLine.back()=='\n'||tLine.back()=='\r'||tLine.back()==' '||tLine.back()=='\t'))tLine.pop_back();
			size_t tStart = tLine.find_first_not_of(" \t\r\n");
			if(tStart!=std::string::npos)tLine = tLine.substr(tStart,15);
			if(!tLine.empty())return String(tLine.c_str());
		}
	...
	void sendClientVersion()
	{
		...
		tBitStream.Write(MessageID(ID_CLIENTVERSION));
		stringCompressor->EncodeString(getRuntimeVersion().c_str(),16,&tBitStream);
	...
	void processNewPlayer(Packet *p)
	{
		...
		// Wire rank codes ("nil","vip","trl","mod","hmd","gm","had","own");
		// legacy "ok."/"mod" tokens still understood.
		bool tIsVip = false;
		{
			const int tRank = KITF_RankFromCode(std::string(tAdminToken));
			if(tRank >= 0)
			{
				tUnit->isAdmin = KITF_RankIsAdmin(tRank);
				tUnit->isMod = KITF_RankIsMod(tRank);
				tIsVip = (tRank == KITF_RANK_VIP);
			}
			else
			{
		...
		// Overhead rank tag follows the presence token (which already hides
		// the rank while the player's tag toggle is off).
		mUnitManager->setRankTag(tUnit, tUser, tUnit->isAdmin, tUnit->isMod, tIsVip);

		String tAddMessage = "";
		if(tIsVip)tAddMessage = " (" USER_DONATOR_TEXT ")";
	...
	void sendNewPlayer(const OwnerToken &target=OwnerToken(TOKEN_NULL))
	{
		...
		stringCompressor->EncodeString(KITF_RankCode(mDef->getVisibleRank()),8,&tBitStream);
	...
	void processLoadChar(Packet *p)
	{
		...
		char tRankCode[8] = "";
		stringCompressor->DecodeString(tRankCode,8,&tBitStream);
		...
		mDef->isMod = tIsMod;
		{
			// Authoritative account rank from the login server overrides the
			// legacy bools above (they are kept on the wire for compat).
			const int tRank = KITF_RankFromCode(std::string(tRankCode));
			if(tRank >= 0)mDef->setRank(tRank);
		}
		unsigned short tVipDaysLeft = 0;
		tBitStream.Read(tVipDaysLeft);
		// Timed VIP notice, shown automatically at login - no command needed.
		if(tVipDaysLeft>0 && mDef->getRank()==KITF_RANK_VIP)
		{
			mChatManager->message("Your VIP status expires in " + StringConverter::toString((int)tVipDaysLeft) + String(tVipDaysLeft==1?" day.":" days."));
		}
		refreshOwnRankTag();
			if(tIsAdmin)
			{
	...
	void sendSetAdmin(const String &targetUser, const String &rankName)
	{
		RakNet::BitStream tBitStream;

		tBitStream.Write(MessageID(ID_SETADMIN));
		stringCompressor->EncodeString(mUnitManager->getPlayer()->getUser().c_str(),16,&tBitStream);
		stringCompressor->EncodeString(targetUser.c_str(),16,&tBitStream);
		stringCompressor->EncodeString(rankName.c_str(),16,&tBitStream);

		// Broadcast: the login server answers, the game server ignores it.
		mPeer->Send(&tBitStream, HIGH_PRIORITY, RELIABLE, 1, UNASSIGNED_SYSTEM_ADDRESS, true);
	}
	void sendSetVip(const String &targetUser, const unsigned short days, const String &reason)
	{
		RakNet::BitStream tBitStream;

		tBitStream.Write(MessageID(ID_SETVIP));
		stringCompressor->EncodeString(mUnitManager->getPlayer()->getUser().c_str(),16,&tBitStream);
		stringCompressor->EncodeString(targetUser.c_str(),16,&tBitStream);
		tBitStream.Write(days);
		stringCompressor->EncodeString(reason.c_str(),64,&tBitStream);

		// Broadcast: the login server answers (via ID_SETADMIN), the game server ignores it.
		mPeer->Send(&tBitStream, HIGH_PRIORITY, RELIABLE, 1, UNASSIGNED_SYSTEM_ADDRESS, true);
	}
	void processSetAdmin(Packet *p)
	{
		RakNet::BitStream tBitStream(p->data, p->length, false);
		MessageID tID;
		bool tSuccess = false;
		...
		tBitStream.Read(tID);
		tBitStream.Read(tSuccess);
		stringCompressor->DecodeString(tMsg,64,&tBitStream);
		mChatManager->message(String(tSuccess ? "Setadmin: " : "Setadmin failed: ") + String(tMsg));
	}
	void refreshOwnRankTag()
	{
		MagixUnit *tP = mUnitManager->getPlayer();
		if(!tP)return;
		mUnitManager->setRankTag(tP,tP->getUser(),(mDef->isAdmin&&mDef->isAdminOn),(mDef->isMod&&mDef->isModOn),KITF_RankIsVip(mDef->getRank()));
	}
	void processRankPush(Packet *p)
	{
		// Live rank update from the login server (no relog needed).
		RakNet::BitStream tBitStream(p->data, p->length, false);
		MessageID tID;
		char tRankCode[8] = "";
		tBitStream.Read(tID);
		...
		mDef->setRank(tRank);
		refreshOwnRankTag();
		if(mUnitManager->getPlayer())
		{
			mUnitManager->destroyParticleOnUnit(mUnitManager->getPlayer(),"Aureola");
			if(mDef->isAdmin)
			{
				mDef->adminAuraColour = ColourValue(0,0,0);
				mUnitManager->createAdminAura(mUnitManager->getPlayer(),mDef->adminAuraColour);
			}
		}
		// Re-announce presence so other players see the new tag at once.
		sendNewPlayer();
		mChatManager->message(String("Your rank is now ") + String(KITF_RankName(tRank)) + String("."));
	}
	...
	void processAnnounce(Packet *p)
	{
		// Live server event: [effect][text]. effect 0 = banner only,
		// 1 = banner + earthquake weather effect.
		RakNet::BitStream tBitStream(p->data, p->length, false);
		MessageID tID;
		unsigned char tEffect = 0;
		char tText[256] = "";
		tBitStream.Read(tID);
		tBitStream.Read(tEffect);
		stringCompressor->DecodeString(tText,256,&tBitStream);

		mChatManager->message(String(tText),CHAT_EVENT);
		mAlertBox->showAlert(String(tText),0.5,0.5,10);
		if(tEffect==1)mSkyManager->setWeatherEffect("Earthquake");
	}
