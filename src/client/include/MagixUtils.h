...
namespace KITF_Utils 
{
	...
	// rewritten.
		vector<String>::type XORInternal(const String inFile)
		{
			// Reads/cipher-inverts a .dat produced by XOR7FileGen(encrypt, checksum).
			// Self-consistent: encrypt summed plaintext bytes; here we reconstruct the
			// same plaintext (preChecksum=false sums post-XOR bytes) so the checksum
			// matches. No lines are swallowed/dropped; the last line is the checksum.
			vector<String>::type raw, out;
			...
			if (in.is_open())
			{
				...
				{
					char tLine[512] = "";
					in.getline(tLine, 512);
					raw.push_back(tLine);
				}
				if (!raw.empty()) raw.pop_back(); // trailing empty line from getline-at-eof
				in.close();

				if (!raw.empty())
				{
					for (size_t i = 0; i < raw.size() - 1; ++i)
						out.push_back(XOR7(raw[i].c_str(), &tChecksum, false));
					const bool tChecksumRight = (tChecksum == StringConverter::parseLong(XOR7(raw.back()).c_str()));
					if (!tChecksumRight)
