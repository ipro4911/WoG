...
public:
	...
	// Overhead rank tag built from the GameConfig.h standard texts
	// ("[ADMIN] Name" / "[MOD] Name" / "[VIP] Name") while the toggle is
	// on, plain username otherwise. No-op without a tag.
	void setRankTag(MagixUnit *target, const String &username, bool showAdmin, bool showMod, bool showVip=false)
	{
		if(!target || !target->getUserTag())return;
		String tCaption = username;
		if(showAdmin)tCaption = String("[") + String(USER_ADMIN_TEXT) + String("] ") + tCaption;
		else if(showMod)tCaption = String("[") + String(USER_MOD_TEXT) + String("] ") + tCaption;
		else if(showVip)tCaption = String("[") + String(USER_VIP_TEXT) + String("] ") + tCaption;
		target->getUserTag()->setCaption(tCaption);
	}
