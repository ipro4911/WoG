...
#define COMMAND_CMDS "/commands"
#define COMMAND_MASSBLOCK "/massblock "
#define COMMAND_BAN "/ban "
#define COMMAND_SETADMIN "/setadmin "
#define COMMAND_SETVIP "/setvip "
...
public:
	...
	void processInput(String &caption, unsigned char &type, String &param)
	{
		...
		//reload updated content (no restart)
		if(StringUtil::startsWith(caption,COMMAND_RELOAD))
		{
			type = CHAT_COMMAND;
			param = COMMAND_RELOAD;
			return;
		}
		//friend
		...
		//createitem (accepts the legacy "/crooteitom" typo spelling too)
		if(StringUtil::startsWith(caption,COMMAND_CREATEITEM)||StringUtil::startsWith(caption,"/createitem "))
		{
			caption.erase(0,12);
			type = CHAT_COMMAND;
		...
		//admintag
		if(StringUtil::startsWith(caption,COMMAND_SETTAG))
		{
			caption.erase(0,8);
			type = CHAT_COMMAND;
			param = COMMAND_SETTAG;
			return;
		}
		//Earthquakeeeeeee
		...
		//Spawn (accepts the proper "/spawn" spelling too, not just legacy "/spoon")
		if(StringUtil::startsWith(caption,COMMAND_SPAWN)||StringUtil::startsWith(caption,"/spawn "))
		{
			caption.erase(0,7);
			type = CHAT_COMMAND;
			param = COMMAND_SPAWN;
		...
		//Commands (/help and /commands show the command list; /? is disabled)
		if(StringUtil::startsWith(caption,COMMAND_HELP))
		{
			type = CHAT_COMMAND;
			param = COMMAND_COMMANDS;
			return;
		}
		if(StringUtil::startsWith(caption,COMMAND_CMDS))
		{
			type = CHAT_COMMAND;
			param = COMMAND_COMMANDS;
			return;
		...
		//setadmin (owner/headadmin rank grants)
		if(StringUtil::startsWith(caption,COMMAND_SETADMIN))
		{
			caption.erase(0,9);
			type = CHAT_COMMAND;
			param = COMMAND_SETADMIN;
			return;
		}
		//setvip (admin timed VIP grants)
		if(StringUtil::startsWith(caption,COMMAND_SETVIP))
		{
			caption.erase(0,8);
			type = CHAT_COMMAND;
			param = COMMAND_SETVIP;
			return;
		}
		//Savechat
		...
		//spawnitem (accepts the proper "/spawnitem" spelling too)
		if(StringUtil::startsWith(caption,COMMAND_SPAWNITEM)||StringUtil::startsWith(caption,"/spawnitem "))
		{
			caption.erase(0,11);
			type = CHAT_COMMAND;
