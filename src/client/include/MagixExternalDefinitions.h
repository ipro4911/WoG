...
public:
	...
	bool isMod;
	bool isModOn;
	bool isAdminOn; // admin nameplate/chat tag toggle (/settag on|off)
	unsigned char playerRank; // KITF_Rank, authoritative from login server
	...
	MagixExternalDefinitions()
	{
		...
		isModOn = false;
		isAdminOn = true;
		playerRank = (unsigned char)KITF_RANK_NORMAL;
		adminAuraColour = ColourValue(0, 0, 0);
	...
	// Account rank (set only from the login server: ID_LOADCHAR / ID_SETRANK).
	// isAdmin/isMod are derived so every existing gate keeps working.
	void setRank(int r)
	{
		playerRank = (unsigned char)KITF_RankNormalize(r);
		isAdmin = KITF_RankIsAdmin(playerRank);
		isMod = KITF_RankIsMod(playerRank);
		if(!isMod) isModOn = false;
		if(!isAdmin) isAdminOn = false;
	}
	int getRank() const { return KITF_RankNormalize(playerRank); }
	// Rank shown to others (presence token): hidden when the tag toggle is off.
	int getVisibleRank() const
	{
		const int r = getRank();
		if(KITF_RankIsAdmin(r) && !isAdminOn) return KITF_RANK_NORMAL;
		if(KITF_RankIsMod(r) && !isModOn) return KITF_RANK_NORMAL;
		return r;
	}
	void initialize();
	...
	void loadSettings(bool& isTablet, Real& mouseSensitivity, Real& musicVol, Real& guiVol, Real& soundVol, bool& isShadowsOn, bool& isBloom, String& username, bool& generalCharname, bool& localUsername, bool& channelBlink, bool& runMode, bool& doubleJumpOn, bool& autoLipSync, bool& useWindowsCursor)
	{
		...
		{
			username = tLine[7];
			// Heal Settings.cfg files written by old launcher builds, which
			// stored "username=<name>" on this line - the prefix would
			// otherwise become part of the login name and fail server-side.
			if(username.length() > 9 && username.substr(0, 9) == "username=")username.erase(0, 9);
		}
	...
	{
		// Main-menu update box: used to dump update.txt here, but that file
		// is now a machine manifest ("KITFUP 1 VER ..."), not human text.
		// Static label instead (change this one string to reword it).
		return String("Beta Open");
	}
	...
	const String loadLocalIP(const String& filename)
	{
		...
		if(inFile.good())
		{
			...
			// trim whitespace/CR/LF/nulls
			while(!line.empty() && (line.back()=='\n' || line.back()=='\r' || line.back()==' ' || line.back()=='\t' || line.back()=='\0')) line.pop_back();
			size_t start = line.find_first_not_of(" \t\r\n");
			if(start != std::string::npos) line = line.substr(start);
			// strip optional :port suffix (Connect takes IP + port separately)
	...
	// Re-parse file-backed definition lists after a live update swapped
	// them on disk. Pass ONLY files that changed (the updater reports them).
	// Deliberately absent: loadUnitMeshes (generated from compiled GMAX_*
	// constants, no file input) and Hotkeys.cfg (user settings, never
	// clobbered). Each loader clears its own lists first, so re-calling is
	// safe and chat/history/data are untouched.
	void reloadUpdatedDefs(const std::vector<std::string> &changed)
	{
		bool tItems = false, tAttacks = false, tCritters = false, tEmotes = false;
		for(size_t i = 0; i < changed.size(); i++)
		{
			std::string b = changed[i];
			size_t s = b.find_last_of("/\\");
			if(s != std::string::npos)b = b.substr(s+1);
			for(size_t j = 0; j < b.size(); j++)
				if(b[j]>='A'&&b[j]<='Z')b[j] = (char)(b[j]-'A'+'a');
			if(b=="items.cfg"||b=="items.dat")tItems = true;
			else if(b=="ad1.dat"||b=="customattacks.cfg")tAttacks = true;
			else if(b=="cd1.dat"||b=="customcritters.cfg")tCritters = true;
			else if(b=="unitemotes.cfg")tEmotes = true;
		}
		if(tItems)loadItems((ENCRYPTED_ITEMS) ? "Items.dat" : "Items.cfg", ENCRYPTED_ITEMS);
		if(tAttacks){attackList.clear();loadAttacks("ad1.dat", false);loadAttacks("CustomAttacks.cfg", true);}
		if(tCritters){critterList.clear();loadCritters("cd1.dat", false);loadCritters("CustomCritters.cfg", true);}
		if(tEmotes)loadUnitEmotes("UnitEmotes.cfg");
	}
