...
protected:
	...
	// launcher auto-login state
	bool autoLogonTriggered;
	bool autoCharTriggered;
	// becomes true after the first successful login: later Multiplayer
	// clicks open the logon form (typable) instead of auto-logging in,
	// so accounts can be switched without restarting the game.
	bool mEverLoggedIn;
	// char-creation name field: previous-frame visibility (edge trigger)
	bool mCharNameWasUp;
...
public:
	MagixGUI()
	{
		...
		mEffectsManager = 0;
		mCamera = 0;
		autoLogonTriggered = false;
		autoCharTriggered = false;
		mEverLoggedIn = false;
		mCharNameWasUp = false;
		mFader = 0;
		mCursor = 0;
		mBackdrop = 0;
		mHPBar = 0;
	...
	void updateNetworkMessage()
	{
		...
		case NETWORK_LOADCHARSUCCESS:
			...
			tryAutoEnterChar(); // launcher auto-char (no-op unless --autochar)
			break;
	...
	void updateLogo(const FrameEvent &evt)
	{
		...
		if(mGameStateManager->getGameState()==GAMESTATE_STARTSCREEN && logoCount<=5)
		{
			...
			if(logoCount>=5)
			{
				...
				mCursor->show();
				// Campaign (New Game) is disabled: keep it hidden and slide
				// the remaining buttons up to fill the gap.
				mButton[BUTTON_NEWGAME]->hide();
				mButton[BUTTON_MULTIPLAYER]->show();
				mButton[BUTTON_MULTIPLAYER]->setPosition(0.22,0.45);
				mButton[BUTTON_QUIT]->show();
				mButton[BUTTON_QUIT]->setPosition(0.22,0.55);
	...
	void updateCharScreen(const FrameEvent &evt)
	{
		...
		// Creation pages show the name field: focus it automatically the
		// moment it appears while empty, so no click is needed. Edge
		// triggered (only on hidden->shown) so it never steals focus back
		// while the player uses sliders/buttons on the same page.
		{
			bool tNameUp = mBoxText[GUI_CHARSCREENBOX] && mBoxText[GUI_CHARSCREENBOX]->isVisible();
			if(tNameUp && !mCharNameWasUp && mCharScreenManager->getName()==""
				&& mInputManager->getInputMode()==INPUT_CONTROL)
			{
				toggleInputMode(false,mBoxText[GUI_CHARSCREENBOX],"Name: ",15,false,"",false);
			}
			mCharNameWasUp = tNameUp;
		}
		}
	}
	...
	void mouseLeftDown()
	{
		...
		{
			// Whole box is a valid target while the name field is up: the
			// text itself is a thin strip that is easy to miss-click.
			if(mBoxText[GUI_CHARSCREENBOX]->isVisible())
	...
	void mouseLeftUp()
	{
		...
		if(isCursorOverButton(mHoverButton))
		{
			...
			if(mHoverButton==mButton[BUTTON_NEWGAME])	//new game (campaign: disabled)
			{
				return;
			}
		if(mHoverButton==mButton[BUTTON_MULTIPLAYER])	//multiplayer game
		{
			...
			// Credentials already known (saved or passed by the launcher):
			// first login of the session goes straight in. After a logout,
			// fall through to the form so a different user/pass can be
			// typed without restarting the game.
			if(!mEverLoggedIn&&logonUsername!=""&&logonPassword!=""){logOn();return;}
			showLogonOverlay(true);
	...
	void toggleInputMode(bool isEnter, OverlayElement *inputBox=0, String defaultCaption="Say: ", unsigned short charLimit=256, bool clearAtReturn=true, String startText="", bool allowTempStorage=true, bool allowNewLine=false, bool hideText=false)
	{
		...
		if(mInputManager->toggleInputMode())
		{
			if(mInputManager->getInputBox()==mBoxText[GUI_TEXTINPUT])
			{
				...
				if(mDef->isAdmin && mDef->isAdminOn)tType = CHAT_ADMIN;
				...
				if(tType == CHAT_COMMAND)
				{
					...
					else if(tParam==COMMAND_MODON && mDef->isMod && !mDef->isModOn)
					{
						...
						// Show the tag live: nameplate + presence rebroadcast.
						mNetworkManager->refreshOwnRankTag();
						mNetworkManager->sendNewPlayer();
					}
					...
					else if(tParam==COMMAND_SETTAG && mDef->isAdmin)
					{
						// /settag on|off - admin nameplate/chat tag toggle.
						String tArg = tCaption;
						StringUtil::toLowerCase(tArg);
						StringUtil::trim(tArg);
						if(tArg=="on"||tArg=="show")
						{
							mDef->isAdminOn = true;
							mChatManager->message("Admin tag on");
							mNetworkManager->refreshOwnRankTag();
							mNetworkManager->sendNewPlayer();
						}
						else if(tArg=="off"||tArg=="hide")
						{
							mDef->isAdminOn = false;
							mChatManager->message("Admin tag off");
							mNetworkManager->refreshOwnRankTag();
							mNetworkManager->sendNewPlayer();
						}
						else mChatManager->message("Usage: /settag [on|off]");
					}
					else if(tParam==COMMAND_SPAWN && mDef->isAdmin)
					{
						if(tCaption=="")mChatManager->message("Usage: /spawn [critter]");
						else
						{
							const Critter tC = mDef->getCritter(tCaption);
							if(tC.type=="")mChatManager->message("Unknown critter: "+tCaption);
							else
							{
					...
					else if(tParam==COMMAND_SETADMIN && KITF_RankCanSetAdmin(mDef->getRank()))
					{
						// /setadmin <name> <rank> - the server re-checks everything.
						const vector<String>::type tPart = StringUtil::split(tCaption," ",1);
						if(tPart.size()<2)mChatManager->message("Usage: /setadmin [name] [normal/vip/trialmod/mod/headmod/gm/headadmin/owner]");
						else mNetworkManager->sendSetAdmin(tPart[0],tPart[1]);
					}
					else if(tParam==COMMAND_SETVIP && mDef->isAdmin)
					{
						// /setvip <name> <30d> [reason] - timed VIP grant, auto-expires.
						const vector<String>::type tPart = StringUtil::split(tCaption," ",2);
						String tDaysArg = (tPart.size()>1 ? tPart[1] : "");
						StringUtil::trim(tDaysArg);
						// Days are typed with a 'd' suffix (30d); plain numbers work too.
						if(tDaysArg.length()>0 && (tDaysArg[tDaysArg.length()-1]=='d' || tDaysArg[tDaysArg.length()-1]=='D'))
							tDaysArg.erase(tDaysArg.length()-1,1);
						const int tDays = (tDaysArg=="" ? 0 : StringConverter::parseInt(tDaysArg));
						if(tPart.size()<2 || tPart[0]=="")mChatManager->message("Usage: /setvip [name] [30d] [reason]");
						else if(tDays<1 || tDays>3650)mChatManager->message("Days must be 1-3650 (e.g. 30d).");
						else mNetworkManager->sendSetVip(tPart[0],(unsigned short)tDays,(tPart.size()>2 ? tPart[2] : ""));
					}
					...
					else if(tParam==COMMAND_COMMANDS)
					{
						...
						if(mDef->isAdmin)
						{
							...
							mChatManager->message("/spawn [critter]");
							mChatManager->message("/spawnitem [name]");
							mChatManager->message("/godspeak [message]");
							mChatManager->message("/settag [on|off]");
							mChatManager->message("/setvip [name] [30d] [reason]");
							mChatManager->message("/ban [name];[num days] (Use as last resort)");
						}
						if(KITF_RankCanSetAdmin(mDef->getRank()))
						{
							mChatManager->message("/setadmin [name] [normal/vip/trialmod/mod/headmod/gm/headadmin/owner]");
						}
						if(mDef->isMod)
						{
		...
		if(object)
		{
			...
			if(flags==CRITTER_MASK)
			{
				...
				if(!isRightClick)
				{
					...
					else if (tCritter->getIsPet() && (mDef->isAdmin))
					{
						const PetFlags* tFlags = mUnitManager->getPlayer()->getPetFlags();
						mButtonText[BUTTON_SELECTBOX1]->setCaption("Pet");
						mButtonText[BUTTON_SELECTBOX2]->setCaption("");
	...
	// launcher auto-login: fill credentials and log in once.
	void tryAutoLogon()
	{
		if (autoLogonTriggered) return;
		if (!MagixAutoLogin::enabled || MagixAutoLogin::user.empty() || MagixAutoLogin::pass.empty()) return;
		autoLogonTriggered = true;
		logonUsername = MagixAutoLogin::user;
		logonPassword = MagixAutoLogin::pass;
		logOn();
	}
	// launcher auto-char: start the game with the current (first) character once it loads.
	void tryAutoEnterChar()
	{
		if (autoCharTriggered) return;
		if (!MagixAutoLogin::enabled || !MagixAutoLogin::autoChar) return;
		autoCharTriggered = true;
		mCharScreenManager->startGame();
	}
	void logOn()
	{
