...
int generateFontTexture(Ogre::String fontName, bool load = false)
{
    ...
    */
    ...
    // new ogre method
    const Ogre::Font::CodePointRangeList& ranges = font->getCodePointRangeList();
    for (const Ogre::Font::CodePointRange& range : ranges)
    {
        for (Ogre::uint32 i = range.first; i <= range.second; ++i)
        {
            try
            {
                const Ogre::Font::GlyphInfo& gi = font->getGlyphInfo(i);

#ifdef FONT_UNICODE
                wchar_t tmp[20];
                swprintf(tmp, 20, L"%lc", i);  // %lc for wide character
#else
                char tmp[20];
                snprintf(tmp, sizeof(tmp), "%c", i);
#endif
