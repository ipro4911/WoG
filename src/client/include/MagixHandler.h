...
public:
	...
	bool update(const FrameEvent &evt)
	{
		...
		switch(mGameStateManager->getGameState())
		{
			...
			case GAMESTATE_STARTGAME:
				// Relog moment: pull anything new (silent when up to date).
				runLiveUpdate(false);
				mGui->showBackdropOverlay(false);
				mGui->showGUI(true);
				mGui->refreshBio();
	...
	// Append-only update notice: chat history is NEVER cleared by updates.
	void liveUpdateNotice(const String &msg, bool alert)
	{
		mGui->getChatManager()->message(msg);
		if(alert)mGui->getAlertBox()->showAlert(msg,0.5,0.5,8);
	}
	// Pull new content without restarting. Data-only updates apply live
	// (new maps work on next enter, reloaded defs apply at once); an
	// exe/dll change means nothing is touched and a restart is requested.
	void runLiveUpdate(bool manual)
	{
		MagixChatManager *tChat = mGui->getChatManager();
		if(manual)tChat->message("Checking for updates...");
		LiveUpdateResult tRes = mLiveUpdater.sync();
		if(!tRes.checked)
		{
			tChat->message("Update check failed: " + String(tRes.error.c_str()));
			return;
		}
		if(tRes.needsRestart)
		{
			liveUpdateNotice("A new version (" + String(tRes.newVersion.c_str()) + ") is ready - restart the game to apply it.",true);
			return;
		}
		if(tRes.upToDate)
		{
			if(manual)tChat->message("Already up to date (v" + String(tRes.newVersion.c_str()) + ").");
			return;
		}
		mDef->reloadUpdatedDefs(tRes.applied);
		liveUpdateNotice("Update applied: " + StringConverter::toString((int)tRes.applied.size()) + " file(s). New maps and items are live now.",true);
		if(!tRes.skipped.empty())
			tChat->message(StringConverter::toString((int)tRes.skipped.size()) + " file(s) are in use and apply on restart.");
	}
	...
	void updateCommand()
	{
		...
		else if(tCommand==COMMAND_RELOAD)
		{
			runLiveUpdate(true);
		}
	...
	void updateStartScreen(const FrameEvent &evt)
	{
		mNetworkManager->update(evt);
		mGui->update(evt);
		// No startup auto-login: the game always rests on the main menu.
		// Auto-login happens only when the player clicks Multiplayer
		// (MagixGUI uses the saved/launcher credentials there).
		//mSoundManager->update(evt);
	}
