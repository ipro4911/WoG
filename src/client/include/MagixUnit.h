...
//mBodyEnt->getParentSceneNode()->showBoundingBox(true);
	...
	void setColours(const ColourValue &pelt, const ColourValue &underfur, const ColourValue &mane, const ColourValue &nose, const ColourValue &eyeTop, const ColourValue &eyeBot, const ColourValue &ears, const ColourValue &tailtip, const ColourValue &eyes, const ColourValue &wing, const ColourValue &marking)
	{
		...
		else if(headMeshID=="7")headMeshID = "1";
	...
	//Ogre 1.7 copyMatchingState() throws on the first animation name that is not
	//shared by both sets. Unit parts can use different rig families now (wolf),
	//so copy only the states that exist on both sides.
	void copyMatchingAnimStates(AnimationStateSet *src, AnimationStateSet *dst)
	{
		if(!src || !dst)return;
		AnimationStateIterator tIt = src->getAnimationStateIterator();
		while(tIt.hasMoreElements())
		{
			AnimationState *tSrc = tIt.getNext();
			const String tName = tSrc->getAnimationName();
			if(dst->hasAnimationState(tName))
			{
				AnimationState *tDst = dst->getAnimationState(tName);
				tDst->setEnabled(tSrc->getEnabled());
				tDst->setLoop(tSrc->getLoop());
				tDst->setTimePosition(tSrc->getTimePosition());
				tDst->setWeight(tSrc->getWeight());
			}
		}
	}
	void replaceHeadMesh(SceneManager *sceneMgr, const String &meshName, bool resetMaterial=false)
	{
		...
		copyMatchingAnimStates(mHeadEnt->getAllAnimationStates(),tHead->getAllAnimationStates());
		...
		copyMatchingAnimStates(tHead->getAllAnimationStates(),mHeadEnt->getAllAnimationStates());
	...
	void replaceBodyMesh(SceneManager *sceneMgr, const String &meshName, bool resetMaterial=false)
	{
		...
		copyMatchingAnimStates(mBodyEnt->getAllAnimationStates(),tBody->getAllAnimationStates());
		...
		copyMatchingAnimStates(tBody->getAllAnimationStates(),mBodyEnt->getAllAnimationStates());
	...
	void replaceManeMesh(SceneManager *sceneMgr, const String &meshName, bool resetMaterial=false)
	{
		...
		copyMatchingAnimStates(mHeadEnt->getAllAnimationStates(),tMane->getAllAnimationStates()); //to avoid mane animation differing from head
		...
		copyMatchingAnimStates(tMane->getAllAnimationStates(),mManeEnt->getAllAnimationStates());
	...
	void replaceTailMesh(SceneManager *sceneMgr, const String &meshName, bool resetMaterial=false)
	{
		...
		copyMatchingAnimStates(mTailEnt->getAllAnimationStates(),tTail->getAllAnimationStates());
		...
		copyMatchingAnimStates(tTail->getAllAnimationStates(),mTailEnt->getAllAnimationStates());
	...
	void replaceWingMesh(SceneManager *sceneMgr, const String &meshName, bool resetMaterial=false)
	{
		...
		copyMatchingAnimStates(mBodyEnt->getAllAnimationStates(),tWing->getAllAnimationStates()); //to avoid wing animation differing fom body
		...
		copyMatchingAnimStates(tWing->getAllAnimationStates(),mWingEnt->getAllAnimationStates());
	...
	void replaceTuftMesh(SceneManager *sceneMgr, const String &meshName, bool resetMaterial=false)
	{
		...
		copyMatchingAnimStates(mBodyEnt->getAllAnimationStates(),tTuft->getAllAnimationStates()); //to avoid tuft animation differing from body
		...
		copyMatchingAnimStates(tTuft->getAllAnimationStates(),mTuftEnt->getAllAnimationStates());
