...
public:
	...
	//Spectrum wolf pairing: Head7 only aligns with Body7 (authored as a set),
	//and needs the WolfMat texture group instead of KovMat.
	void pairWolfMeshes()
	{
		const bool tIsWolf = (meshName("Head",headID)=="Head7");
		const vector<String>::type tMesh = StringUtil::split(mUnit->getMeshString(),";",5);
		const String tBody = (tMesh.size()==5?tMesh[0]:"Body");
		if(tIsWolf && tBody!="Body7")mUnit->replaceBodyMesh(mSceneMgr,"Body7",(page==PAGE_PRESET1||page==PAGE_PRESET2));
		if(!tIsWolf && tBody=="Body7")mUnit->replaceBodyMesh(mSceneMgr,"Body",(page==PAGE_PRESET1||page==PAGE_PRESET2));
		const String tMat = mUnit->getMatGroupName();
		if(tIsWolf && tMat=="KovMat")mUnit->setMaterial("WolfMat");
		if(!tIsWolf && tMat=="WolfMat")mUnit->setMaterial("KovMat");
	}
		...
		if (page == PAGE_SELECT)
		{
			...
			else
			{
				if (charID == numChars)
				{
					...
					else if (getNumChars() >= 20)
					{
						mAlertBox->showAlert("You already have\n20 characters!", (Real)(0.85 - mNextButton->getWidth() + mRButton[0]->getWidth()), 0.85, 5);
						mNextButton->hide();
					}
				}
	...
	{
		const bool tIsWolf = (meshName("Head",headID)=="Head7");
		mUnit->createUnit(1,mSceneMgr,(tIsWolf?"Body7":"Body"),meshName("Head",headID),meshName("Mane",maneID),meshName("Tail",tailID),meshName("Wing",wingID),"KovMat",meshName("Tuft",tuftID));
		if(tIsWolf)mUnit->setMaterial("WolfMat");
		positionUnitForEdit();
	}
