...
protected:
	...
	unsigned char clientRank[MAX_CLIENTS];
...
public:
	ServerManager()
	{
		...
		for(int i=0;i<MAX_CLIENTS;i++)
		{
			...
			clientMap[i] = "";
			clientName[i] = "";
			clientRank[i] = (unsigned char)KITF_RANK_NORMAL;
	...
	void runLoop()
	{
		...
		if (_kbhit())
		{
			// Line-based console: type a command + Enter. Single letters
			// keep working (Q/S/C/H/V/L/M). Unknown input prints help
			// instead of vanishing into per-key _getch() probes.
			char input[512]="";
			gets_s(input);
			message = input;
			...
			if(message=="version" || message=="V")
			{
				printf("Version: 0.719\n");
				continue;
			}
			if(message=="list" || message=="L")
			{
			...
			{
				puts("Commands: quit(Q) stat(S) chat(C) hide(H) version(V) list(L) mem(M)");
				puts("Account admin (ranks/bans) runs on the LOGINSERVER console,");
				puts("not here: setadmin <user> <rank>, banname <user> [days].");
				continue;
			}
			if(!message.empty())printf("Unknown command '%s'. Type help.\n",message.c_str());
		}
	/*
			{
				...
				if (message=="commands")
				{
					...
					if(!hideText)printf("Command packet logging %s.\n",showCommands?"on":"off");
					continue;
				}
				if (message=="showchat")
				{
...
/*		if (kbhit())
		{
			...
			*/
			...
			switch (packetIdentifier)
			{
				...
				case ID_CLIENTVERSION:
					{
						RakNet::BitStream tReceiveBit(p->data, p->length, false);
						MessageID tMessage;
						char tVersion[17] = "";
						tReceiveBit.Read(tMessage);
						stringCompressor->DecodeString(tVersion,17,&tReceiveBit);

						const string tExpected = getExpectedVersion();
						if(strcmp(tVersion, tExpected.c_str())!=0)
						{
							if(!hideText)printf("Client %s has version '%s', expected '%s'. Rejecting.\n",
								p->systemAddress.ToString(), tVersion, tExpected.c_str());

							RakNet::BitStream tBitStream;
							tBitStream.Write(MessageID(ID_INVALID_PASSWORD));
							server->Send(&tBitStream, HIGH_PRIORITY, RELIABLE, 0, p->systemAddress, false);

							server->CloseConnection(p->systemAddress, true, 0);
							tCont = false;
						}
					}
					break;
				...
				case ID_TOKENCONNECTED:
					...
					if(p->systemAddress==mainServerAdd)
					{
						...
						OwnerToken tToken;
						SystemAddress tAdd;
						unsigned short tDayTime = 0;
						unsigned short tWeatherTime = 0;
						char tRankCode[8] = "";

						tReceiveBit.Read(tMessage);
						tReceiveBit.Read(tToken);
						tReceiveBit.Read(tAdd);
						tReceiveBit.Read(tDayTime);
						tReceiveBit.Read(tWeatherTime);
						// Lenient: older login servers omit the rank tail.
						stringCompressor->DecodeString(tRankCode,8,&tReceiveBit);

						// Record the authoritative rank for per-command auth.
						{
							const int tRank = KITF_RankFromCode(string(tRankCode));
							for(int i=0;i<MAX_CLIENTS;i++)
							{
								if(clientAdd[i]==tAdd)
								{
									clientRank[i] = (unsigned char)(tRank>=0 ? tRank : KITF_RANK_NORMAL);
									break;
								}
							}
						}

						// Forward to the player WITHOUT the rank tail: the client
						// already learned its rank at character load (ID_LOADCHAR),
						// so its packet layout is unchanged.
						RakNet::BitStream tBitStream;
						tBitStream.Write(MessageID(tMessage));
						tBitStream.Write(tToken);
						tBitStream.Write(tDayTime);
						tBitStream.Write(tWeatherTime);
					...
					break;
				case ID_SETRANK:
					// Live rank sync from the login server: [address][rankcode].
					// Updates the stored authority rank (no forward: the target
					// client already got its own copy directly from login).
					if(p->systemAddress==mainServerAdd)
					{
						RakNet::BitStream tReceiveBit(p->data, p->length, false);
						MessageID tMessage;
						...
						if(tRank>=0)
						{
							for(int i=0;i<MAX_CLIENTS;i++)
							{
								if(clientAdd[i]==tAdd)
								{
									clientRank[i] = (unsigned char)tRank;
									if(showCommands)printf("[RANK] '%s' is now %s (live sync).\n",clientName[i].length()>0?clientName[i].c_str():tAdd.ToString(),KITF_RankName(tRank));
									break;
								}
							}
						}
					}
					else if(showCommands)printf("[CMD] Ignored ID_SETRANK from %s (login only).\n",p->systemAddress.ToString());
					break;
				case ID_FORCELOGOUT:
					{
						RakNet::BitStream tReceiveBit(p->data, p->length, false);
				...
				case ID_KICK:
					{
						// Per-command auth: directly connected clients need a
						// moderator rank; packets relayed from other servers or
						// login were vetted at ingress. Checked BEFORE relay so a
						// forged kick neither executes nor propagates.
						{
							const int tRank = senderRank(p);
							if(tRank>=0 && !KITF_RankCanModerate(tRank))
							{
								if(showCommands)printf("[CMD] denied ID_KICK from '%s' (rank %s).\n",cmdSenderName(p).c_str(),KITF_RankName(tRank));
								break;
							}
						}
						...
						const string tKickBy = cmdSenderName(p);
						OwnerToken tToken = getTokenByName(tName);
						if(tToken==0)
						{
							if(showCommands)printf("[CMD] recv ID_KICK: '%s' kicked '%s' (target offline).\n",tKickBy.c_str(),tName);
							break;
						}
						...
						if(clientIsMine[tToken-1])
						{
							...
							// Moderators may only issue 1-day bans (mirrors the
							// client gate); longer bans need a full admin rank.
							if(tIsBanned)
							{
								const int tRank = senderRank(p);
								if(tRank>=0 && !KITF_RankIsAdmin(tRank))tNumDays = 1;
							}
							if(showCommands)
							{
				...
				case ID_GODSPEAK:
					{
						// Per-command auth: godspeak needs a full admin rank.
						if(p->systemAddress!=mainServerAdd)
						{
							const int tRank = senderRank(p);
							if(tRank>=0 && !KITF_RankIsAdmin(tRank))
							{
								if(showCommands)printf("[CMD] denied ID_GODSPEAK from '%s' (rank %s).\n",cmdSenderName(p).c_str(),KITF_RankName(tRank));
								break;
							}
						}
						RakNet::BitStream tReceiveBit(p->data, p->length, false);
				...
				case ID_INFO:
					{
						...
						// Per-command auth for the staff events: earthquake needs a
						// full admin rank, massblock needs a moderator rank.
						// Relayed packets (other servers) were vetted at ingress.
						// Other event types pass through untouched.
						if(packetIdentifier==ID_EVENT && imTheOwner(p))
						{
							const int tRank = senderRank(p);
							if(tRank>=0)
							{
								bool tAllowed = true;
								if(tType==EVENT_EARTHQUAKE)tAllowed = KITF_RankIsAdmin(tRank);
								else if(tType==EVENT_MASSBLOCK)tAllowed = KITF_RankCanModerate(tRank);
								if(!tAllowed)
								{
									if(showCommands)printf("[CMD] denied ID_EVENT type %u from '%s' (rank %s).\n",(unsigned)tType,cmdSenderName(p).c_str(),KITF_RankName(tRank));
									break;
								}
							}
						}

						// Command-driven events only (earthquake / massblock).
						if(showCommands && packetIdentifier==ID_EVENT)
						{
							string tEvBy = cmdSenderName(p);
							if(tReadSenderToken && tSender>0 && tSender<=MAX_CLIENTS && clientName[tSender-1].length()>0)
								tEvBy = clientName[tSender-1];
							printf("[CMD] recv ID_EVENT type %u from '%s'.\n",(unsigned)tType,tEvBy.c_str());
						}
				...
				case ID_FINDPLAYER:
					{
						// Per-command auth: /where and /whois need a moderator rank.
						{
							const int tRank = senderRank(p);
							if(tRank>=0 && !KITF_RankCanModerate(tRank))
							{
								if(showCommands)printf("[CMD] denied ID_FINDPLAYER from '%s' (rank %s).\n",cmdSenderName(p).c_str(),KITF_RankName(tRank));
								break;
							}
						}
						RakNet::BitStream tReceiveBit(p->data, p->length, false);
						...
						if(showCommands)
						{
							if(tIsToken)printf("[CMD] recv ID_FINDPLAYER (whois token %u) from '%s'.\n",(unsigned)tToken,cmdSenderName(p).c_str());
							else printf("[CMD] recv ID_FINDPLAYER (where '%s') from '%s'.\n",tName.c_str(),cmdSenderName(p).c_str());
						}
	...
	void unassignToken(const OwnerToken &token)
	{
		...
		clientMap[token-1] = "";
		clientName[token-1] = "";
		clientRank[token-1] = (unsigned char)KITF_RANK_NORMAL;
	...
	void assignPlayerData(const OwnerToken &token, bool isMine, Packet *p, const string &playerName, const string &mapName, const unsigned char &dimension/*, const unsigned short &charID*/)
	{
		if(token>0 && token<=MAX_CLIENTS)
		{
			...
			// Fail closed: no powers until the login server reports the rank
			// (ID_TOKENCONNECTED reply / ID_SETRANK).
			clientRank[token-1] = (unsigned char)KITF_RANK_NORMAL;
		}
	}
	...
	// Expected client version, authority = the updater: version.txt in the
	// served client folder (stamped by manifest_build). The compiled
	// GAME_VERSION is only a fallback for dev runs without that folder.
	// Read per check (tiny file, only on client join) so bumping the
	// version never needs a server recompile or restart.
	string getExpectedVersion()
	{
		string tRoot = UPDATE_FILES_DIR;
		if(!tRoot.empty() && tRoot[tRoot.size()-1]!='\\')tRoot += '\\';
		std::ifstream tFile((tRoot+"version.txt").c_str());
		if(tFile.good())
		{
			string tLine;
			std::getline(tFile,tLine);
			while(!tLine.empty() && (tLine[tLine.size()-1]=='\n'||tLine[tLine.size()-1]=='\r'||tLine[tLine.size()-1]==' '||tLine[tLine.size()-1]=='\t'))tLine.erase(tLine.size()-1);
			size_t tStart = tLine.find_first_not_of(" \t\r\n");
			if(tStart!=string::npos)tLine = tLine.substr(tStart,15);
			if(!tLine.empty())return tLine;
		}
		return GAME_VERSION;
	}
	// Authority rank of a directly connected client (KITF_Rank 0..7), as last
	// reported by the login server. Returns -1 for packets that did not come
	// from one of our own clients (login/another server: trusted relay path).
	// Slots default to NORMAL and reset on unassign, so unknown clients have
	// no powers (fail closed).
	int senderRank(Packet *p)
	{
		if(!imTheOwner(p))return -1;
		const OwnerToken tTok = getOwnerToken(p);
		if(tTok<=0 || tTok>MAX_CLIENTS)return KITF_RANK_NORMAL;
		return KITF_RankNormalize(clientRank[tTok-1]);
	}
	// Human-readable sender label for command logging: player name when the
	// packet came from one of our connected clients, else the source role.
	string cmdSenderName(Packet *p)
	{
		if(p->systemAddress==mainServerAdd)return "loginserver";
		for(int i=0;i<MAX_CLIENTS;i++)
		{
			if(clientAdd[i]==p->systemAddress)
				return clientName[i].length()>0 ? clientName[i] : p->systemAddress.ToString();
		}
		for(int i=0;i<MAX_SERVERS;i++)
		{
			if(serverAdd[i]==p->systemAddress)return "server";
		}
		return p->systemAddress.ToString();
	}
